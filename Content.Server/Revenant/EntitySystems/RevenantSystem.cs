...
public sealed partial class RevenantSystem : EntitySystem
{
    ...
    private void OnDamage(EntityUid uid, RevenantComponent component, ref DamageDealtEvent args)
    {
        if (!HasComp<CorporealComponent>(uid))
            return;

        var essenceDamage = args.Damage.GetTotal().Float() * component.DamageToEssenceCoefficient * -1;
