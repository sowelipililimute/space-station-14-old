...
";
        ...
        public async Task TestDamageableComponents()
        {
            ...
            await server.WaitAssertion(() =>
            {
                ...
                sDamageableSystem.ChangeDamage(uid, new Attack(damage, true));
                ...
                // Heal
                sDamageableSystem.ChangeDamage(uid, new Attack(-damage));
                ...
                // Test 'wasted' healing
                sDamageableSystem.ChangeDamage(uid, new Attack(new DamageSpecifier(type3a, 5)));
                sDamageableSystem.ChangeDamage(uid, new Attack(new DamageSpecifier(type3b, 7)));
                sDamageableSystem.ChangeDamage(uid, new Attack(new DamageSpecifier(group3, -11)));
                ...
                // Test Over-Healing
                sDamageableSystem.ChangeDamage(uid, new Attack(new DamageSpecifier(group3, FixedPoint2.New(-100))));
                Assert.That(sDamageableComponent.TotalDamage, Is.EqualTo(FixedPoint2.Zero));

                // Test that if no health change occurred, returns false
                sDamageableSystem.ChangeDamage(uid, new Attack(new DamageSpecifier(group3, -100)));
                Assert.That(sDamageableComponent.TotalDamage, Is.EqualTo(FixedPoint2.Zero));
            });
