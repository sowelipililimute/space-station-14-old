...
public sealed class ShowHealthIconsSystem : EquipmentHudSystem<ShowHealthIconsComponent>
{
    ...
    {
        if (!DamageContainers.Contains(entity.Comp.InjuryContainer))
        {
        ...
        if (entity.Comp.InjuryContainer == "Biological")
        {
            if (TryComp<MobStateComponent>(entity, out var state))
            {
                // Since there is no MobState for a rotting mob, we have to deal with this case first.
                if (HasComp<RottingComponent>(entity) && _prototypeMan.Resolve(entity.Comp.RottingIcon, out var rottingIcon))
                    result.Add(rottingIcon);
                else if (entity.Comp.HealthIcons.TryGetValue(state.CurrentState, out var value) && _prototypeMan.Resolve(value, out var icon))
                    result.Add(icon);
            }
        }
