using System.Linq;
using Content.Client._Offbrand.NuBody;
...
public sealed class LobbyUIController : UIController, IOnStateEntered<LobbyState>, IOnStateExited<LobbyState>
{
    ...
    [UISystemDependency] private readonly OFMVisualBodySystem _visualBody = default!;
    ...
    public EntityUid LoadProfileEntity(HumanoidCharacterProfile? humanoid, JobPrototype? job, bool jobClothes)
    {
        ...
        {
            var dummy = _prototypeManager.Index<SpeciesPrototype>(humanoid.Species).OFMAppearancePrototype;
            dummyEnt = EntityManager.SpawnEntity(dummy, MapCoordinates.Nullspace);
            _visualBody.ApplyProfileTo(dummyEnt, humanoid);
        }
        else
        {
            dummyEnt = EntityManager.SpawnEntity(_prototypeManager.Index<SpeciesPrototype>(HumanoidCharacterProfile.DefaultSpecies).OFMAppearancePrototype, MapCoordinates.Nullspace);
        }
